...
wake_ref = asr.embed_bank(["gordon"])
...
def vision_loop():
    ...
    while True:
        ...
        if info["motors"] is not None:
            print(info["motors"], info["claw_open"])
        with state_lock:
...
try:
    while True:
        ...
        try:
            while True:
                ...
                if task_type == "wake" and text:
                    ...
                    chosen_from = "keyword/exact" if label else None
                    if label is None:
                        sims = {k: asr.best_sim_against(v, nt) for k, v in intent_refs.items()}
                        label = max(sims, key=sims.get) if sims else "idle"
                        if sims.get(label,0.0) < 0.70: label="idle"
                    ui_state.last_transcript = text if text else "(no speech)"
                    ui_state.transcript_until = time.monotonic() + 4.0
                    if label == "mirror":
                        set_phase("mirror"); ui_state.flash_message="MIRROR MODE ACTIVATED"; ui_state.flash_until=time.monotonic()+1.5
                    else:
                        set_phase("idle"); ui_state.flash_message="IDLE MODE"; ui_state.flash_until=time.monotonic()+1.0
        except queue.Empty:
            pass
        ...
        if base is not None:
            frame = ann if (phase=="mirror" and ann is not None) else base
